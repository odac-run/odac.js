...
class Migration {
  ...
  _generateSchemaFileContent(tableName, columns, indexes) {
    const lines = []
    lines.push(`// Schema definition for '${tableName}' — auto-generated by ODAC snapshot`)
    ...
    for (let i = 0; i < colEntries.length; i++) {
      ...
      parts.push(`type: '${mappedType}'`)

      if (meta.maxLength) parts.push(`length: ${meta.maxLength}`)
      if (meta.nullable === false) parts.push('nullable: false')
      if (meta.defaultValue !== null && meta.defaultValue !== undefined) {
        const defaultStr = typeof meta.defaultValue === 'string' ? `'${meta.defaultValue}'` : meta.defaultValue
        parts.push(`default: ${defaultStr}`)
      }

      const comma = i < colEntries.length - 1 ? ',' : ''
      lines.push(`    ${colName}: {${parts.join(', ')}}${comma}`)
    }
